...
function __privateEnvsInstallerDir()
{
  ...
  export STACK_APPLICATIONS_DOCKER_DIR=${STACK_APPLICATIONS_DIR}/pipelines
  ...
  if [[ -d ${STACK_APPLICATIONS_DOCKER_DIR} ]]; then
    export STACK_INSTALLER_PIPELINE_DIR=${STACK_APPLICATIONS_DOCKER_DIR}
  else
    unset STACK_INSTALLER_PIPELINE_DIR
  fi

  #INSTALLER DIR
  if [[ ${STACK_INSTALLER_PIPELINE_DIR} == "" ]]; then
    export STACK_INSTALLER_PIPELINE_DIR=${INSTALLER_DIR}/pipelines
  fi

  export STACK_INSTALLER_DOCKER_CONF_DIR=${STACK_INSTALLER_PIPELINE_DIR}/conf
  export STACK_INSTALLER_DOCKER_FILE_DIR=${STACK_INSTALLER_PIPELINE_DIR}/dockerfiles
  export STACK_INSTALLER_DOCKER_COMPOSE_DIR=${STACK_INSTALLER_PIPELINE_DIR}/compose
