...
function __privateBuildDefault()
{
  ...
  export BUILD_CONTAINER_NAME=${STACK_PREFIX}-${APPLICATION_NAME}
  ...
  export BUILD_APP_BIN_SRC_DIR=${STACK_APPLICATIONS_SOURCE_DIR}/${BUILD_APP_DNS}
  export BUILD_APP_DATA_DIR=${STACK_APPLICATIONS_SOURCE_DIR}/${BUILD_CONTAINER_NAME}
  ...
  if [[ ${APPLICATION_DEPLOY_HOSTNAME} == "" ]]; then
    export APPLICATION_DEPLOY_HOSTNAME=${BUILD_CONTAINER_NAME}
  fi
  ...
  if [[ ${APPLICATION_DEPLOY_DATA_DIR} == "" ]]; then
    export APPLICATION_DEPLOY_DATA_DIR=${BUILD_APP_DATA_DIR}
  fi
